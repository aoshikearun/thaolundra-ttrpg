...
def api_especies_por_categoria():
    ...
    try:
        # Mapeamento de espécies permitidas por categoria
        ESPECIES_POR_CATEGORIA = {
            1: [1, 2, 3, 4, 5],      # Padrão: Bestial, Humano, Klaveck, Darch, Sombra
            2: [1, 2, 3, 4, 5],      # Épica: mesmas da Padrão
            3: [1, 2, 3, 4, 5, 6],   # Especial: inclui Mecha (id 6)
            4: [7],                   # Rokan: apenas Rokan (id 7)
            5: [10, 11],             # Divina: Demônio (10), Anjo (11)
            6: [8, 9]                # Pura: Yun-hai (8), Kyongun (9)
        }

        especies_ids = ESPECIES_POR_CATEGORIA.get(categoria_id, [])

        if not especies_ids:
            return jsonify({'success': False, 'message': 'Categoria sem espécies configuradas'}), 400

        placeholders = ','.join(['%s'] * len(especies_ids))
        cursor.execute(f"""
            SELECT id, nome, minimo_poder, maximo_poder, descricao, 
                   especificidades_mecanicas, advanced, ordem
            FROM especies 
        ...
        especies = cursor.fetchall()
        return jsonify({'success': True, 'especies': especies, 'permite_avancadas': categoria_id in [1, 2, 3]})
...
@criar_ficha_bp.route('/api/ficha/tecnicas/multiples', methods=['POST'])
@login_required
def api_tecnicas_multiples():
    """API: Buscar técnicas de múltiplas fontes (para fontes híbridas)"""
    data = request.get_json()
    fontes_ids = data.get('fontes_ids', [])

    if not fontes_ids:
        return jsonify({'success': False, 'message': 'Nenhuma fonte informada'}), 400

    # Remove duplicatas e valores nulos
    fontes_ids = list(set([int(f) for f in fontes_ids if f]))

    if not fontes_ids:
        return jsonify({'success': False, 'message': 'IDs de fonte inválidos'}), 400
    ...
    try:
        cursor.execute(f"""
            SELECT id, nome, descricao_geral, categoria, especializacao, 
                   fonte_de_poder_id, efetividade, custo
            FROM tecnicas
            WHERE fonte_de_poder_id IN ({placeholders}) AND especializacao = 0
            ORDER BY 
                CASE categoria
                    WHEN 'Inata' THEN 1
                    WHEN 'Básica' THEN 2
        ...
        logger.error(f"Erro ao buscar técnicas múltiplas: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500
...
def api_salvar_ficha():
    ...
    try:
        ...
        faccao_id = data.get('faccao_id')
        if faccao_id is None or faccao_id == 0:
            faccao_id = None

        aura_id = data.get('aura_id')
        if aura_id is None or aura_id == 0:
            aura_id = None
