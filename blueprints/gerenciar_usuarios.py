...
Níveis de acesso: Moderador (7+) e Administrador (9)
"""
from flask import Blueprint, render_template, jsonify, request, flash, redirect, url_for
from flask_login import login_required, current_user
from database import get_db_connection, execute_query
from constants import NIVEIS_CREDENCIAL
import logging
...
def pode_editar_usuario(usuario_id, usuario_nivel=None):
    ...
    # Administrador (9) pode tudo
    if current_user.nivel_credencial >= 9:
        return True, None

    # Moderador (7) não pode editar admins
    if current_user.nivel_credencial >= 7:
        if usuario_nivel is not None and usuario_nivel >= 9:
...
def gerenciar_usuarios():
    ...
    if current_user.nivel_credencial < 7:
        flash("Acesso negado - Nível 7+ requerido", "error")
...
def api_atualizar_usuario(usuario_id):
    ...
    try:
        ...
        if 'nivel_credencial' in data:
            ...
            # Moderador (7) não pode promover acima de 8
            if current_user.nivel_credencial < 9 and novo_nivel > 8:
                return jsonify({'success': False, 'message': 'Moderadores só podem promover até nível 8'}), 403
            # Não pode promover a si mesmo acima de 8 se for moderador
            if usuario_id == current_user.id and current_user.nivel_credencial < 9 and novo_nivel > current_user.nivel_credencial:
...
def api_excluir_usuario(usuario_id):
    """
    API: Excluir usuário (apenas Administradores nível 9)
    O front espera: { success, message }
    """
    if current_user.nivel_credencial < 9:
        return jsonify({'success': False, 'message': 'Acesso negado - Nível 9 requerido'}), 403
    ...
    try:
        ...
        # Remover de mesas
        execute_query("DELETE FROM mesa_online WHERE usuario_id = %s", (usuario_id,), commit=True)
