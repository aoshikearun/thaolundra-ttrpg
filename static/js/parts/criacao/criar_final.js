...
function salvarFicha() {
    ...
    // ✅ CORREÇÃO: faccao_id e aura_id - usar NULL em vez de 0
    let faccaoId = parseInt(document.getElementById('faccao')?.value || 0);
    if (faccaoId === 0) faccaoId = null;  // NULL em vez de 0

    let auraId = parseInt(document.getElementById('aura')?.value || 0);
    if (auraId === 0) auraId = null;  // NULL em vez de 0
    ...
    const fichaCompleta = {
        ...
        faccao_id: faccaoId,  // ✅ Agora é null ou um ID válido
        aura_id: auraId,      // ✅ Agora é null ou um ID válido
...
window.abrirModalCaracteristica = abrirModalCaracteristica;
window.fecharModalCaracteristica = fecharModalCaracteristica;
window.adicionarCaracteristica = adicionarCaracteristica;
window.removerCaracteristica = removerCaracteristica;
window.abrirModalCriarFaccao = abrirModalCriarFaccao;
window.fecharModalFaccao = fecharModalFaccao;
window.criarFaccao = criarFaccao;
window.abrirModalCriarAura = abrirModalCriarAura;
window.fecharModalAura = fecharModalAura;
window.criarAura = criarAura;
window.carregarPassoOito = carregarPassoOito;
