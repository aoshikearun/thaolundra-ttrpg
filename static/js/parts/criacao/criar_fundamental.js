...
if (typeof window._criarFundamentalLoaded === 'undefined') {
    window._criarFundamentalLoaded = true;
    ...
    async function carregarEspeciesPorCategoria(categoriaId) {
        try {
            ...
            if (data.success) {
                ...
                if (typeof window.updateCarouselSpecies === 'function') {
                    window.updateCarouselSpecies(data.especies);
                }
            }
        } catch (error) {
    ...
    window.carregarFontesPoderFicha = function(especieId = null) {
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Aplicar regra de fontes híbridas da categoria
                    const categoria = window.categoriaData;
                    const permiteHibridas = categoria?.permite_fontes_hibridas === true || categoria?.permite_fontes_hibridas === 1;

                    let fontesFiltradas = data.fontes;
                    if (!permiteHibridas) {
                        fontesFiltradas = data.fontes.filter(fonte => fonte.hybrid !== 1 && fonte.hybrid !== true);
                        console.log(`🔧 Categoria não permite fontes híbridas. Total: ${data.fontes.length}, Filtradas: ${fontesFiltradas.length}`);
                    }

                    // ✅ ATUALIZAR TAMBÉM O fontesData DO CARROSSEL
                    if (typeof window.updateCarouselFontes === 'function') {
                        window.fontesData = fontesFiltradas;
                        window.updateCarouselFontes(fontesFiltradas);
                    }
                } else {
    ...
    function confirmarSelecaoEspecie() {
        ...
        window.carregarFontesPoderFicha(selectedSpeciesId);
    ...
    function confirmarSelecaoFonte() {
        if (!selectedFonteId || !selectedFonteData) {
            alert('Selecione uma fonte de poder primeiro.');
            return;
        }

        console.log('✅ Confirmando fonte:', selectedFonteData.nome_exibicao);
        ...
        // ✅ ATUALIZAR A FONTE NO SISTEMA DE TÉCNICAS
        if (typeof window.setSelectedFonte === 'function') {
            window.setSelectedFonte(selectedFonteId, selectedFonteData);
        }
    ...
    window.carregarFontesPoderFicha = carregarFontesPoderFicha;
