...
if (typeof window._criarFichaLoaded === 'undefined') {
    window._criarFichaLoaded = true;
    ...
    let tecnicasDisponiveis = { inatas: [], basicas: [], avancadas: [] };
    ...
    let fichaData = {
        ...
        especie: null,
        fontePoder: null,
        atributos: { forca: 1, destreza: 1, inteligencia: 1, constituicao: 1, poder: 1, aparencia: 1 },
        pericias: [],
        tecnicas: [],
        caracteristicas: { qualidades: [], defeitos: [], outras: [] },
        ocupacao: '',
        moradia: '',
        faccao_id: 0,
        aura_id: null,
        ...
        nome_completo: '',
        privado: false,
        pontos: { pericias: 0, tecnicas: 0, forca_vital: 0, poder_elemental: 0 }
    };
    ...
    const atributoParaIndice = { forca: 0, destreza: 1, inteligencia: 2, constituicao: 3 };
    ...
    class Atributo {
        ...
        getPontosDisponiveis() { return this.pointsCounters[this.meuIndex]; }
        getTotalRecebido() {
            let total = 0;
    ...
    function prevStep() {
        if (typeof currentStep !== 'undefined' && currentStep > 1) nextStep(currentStep - 1);
    }

    function validarPasso(step) {
        switch(step) {
            case 1:
                if (!window.categoriaSelecionada) {
            ...
            case 3:
                ...
                return true;
            case 4:
                if (!window.atributosConfirmados) {
                    alert('Confirme os atributos.');
                    return false;
                }
                if (!window.poderAceito) {
                    alert('Role e aceite o Poder.');
                    return false;
                }
                if (!window.aparenciaRolada) {
                    alert('Role a Aparência.');
                    return false;
                }
                return true;
            case 9:
    ...
    window.atributosTentativas = atributosTentativas;
    window.poderTentativas = poderTentativas;
    ...
    window.tecnicaInataSelecionada = tecnicaInataSelecionada;
    window.tecnicaAvancadaSelecionada = tecnicaAvancadaSelecionada;
    window.tecnicaUltimaSelecionada = tecnicaUltimaSelecionada;
    window.tecnicasSelecionadas = tecnicasSelecionadas;
