...
class ThemeManager {
    ...
    applyPrimaryColor() {
        document.documentElement.style.setProperty('--primary-hue', `${this.primaryHue}deg`);
        this.applyFilterToLogos();
    }

    applyFilterToLogos() {
        const logos = document.querySelectorAll('.logo, .logo-png, [data-primary-logo]');
        logos.forEach(logo => {
            logo.style.filter = `hue-rotate(${this.primaryHue}deg)`;
        });
    }
